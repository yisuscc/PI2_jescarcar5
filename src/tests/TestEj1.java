...
public class TestEj1 {
	...
	private static Integer nMaxBigInt = 50; // n máximo para el fibonacci recursivo sin memoria
	private static Integer nMaxDouble = 100000; // n máximo para el fibonacci no exponencial
	...
	private static List<Trio<Function<Integer, Number>, TipoAjuste, String>> metodosDeDouble = List.of(
			Trio.of(Ejercicio1::facIterDouble, TipoAjuste.LINEAL, "Iterativo double (lineal)"),
			Trio.of(Ejercicio1::facRdouble, TipoAjuste.LINEAL, "Recursivo double (lineal)"));
	...
	// TODO la versión de Big Integer
	...
	private static <E> BigInteger warmup(Function<E, Number> fun, Integer n) {
		...
		for (int i=0; i<numIterWarmup; i++) {
			if (fun.apply((E) n).equals(z)) z.add(BigInteger.ONE);
		}
